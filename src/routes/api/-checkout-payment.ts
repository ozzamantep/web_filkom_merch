...
export const processCheckoutPayment = createServerFn(
  { method: 'POST' },
  async (details: CheckoutPaymentRequest) => {
    try {
      // Dynamic imports to avoid bundling for browser
      const { execute, queryOne, query } = await import('../../lib/database');

      console.log('═══════════════════════════════════════════');
      console.log('🔄 [processCheckoutPayment] EXECUTING');
      ...
      console.error('❌ [processCheckoutPayment] Error:', msg);
