...
class BluetoothThermalPrinter {
  ...
  async connect(deviceId: string): Promise<boolean> {
    try {
      ...
      const characteristic = await service.getCharacteristic('00002ad1-0000-1000-8000-00805f9b34fb');
  ...
  // Print text
  private async printText(text: string, encoding: BufferEncoding = 'utf8'): Promise<void> {
  ...
  async printReceipt(data: ReceiptData): Promise<void> {
    try {
      ...
      // Cut paper (optional - many printers support this)
      // const cutCmd = new Uint8Array([0x1D, 0x56, 0x41, 0x03]); // GS V A n
      // await this.sendCommand(cutCmd);
