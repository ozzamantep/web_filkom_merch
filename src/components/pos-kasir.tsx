...
import type { CreateSaleInput } from "@/routes/api/-admin";
import { createSale, getPaymentMethods } from "@/routes/api/-admin";
import { bluetoothPrinter, type ReceiptData } from "@/lib/bluetooth-printer";
import type { ProductWithVariants } from "@/lib/server-actions";
...
export function POSKasir({ admin_id, admin_name, store_name }: POSKasirProps) {
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      const result = (await createSale(saleInput)) as any;
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-2 max-h-[600px] overflow-y-auto">
              {filteredProducts.map((product) => (
                <button
                  ...
                  className="p-3 border border-border rounded-lg hover:bg-accent hover:border-primary transition"
                >
